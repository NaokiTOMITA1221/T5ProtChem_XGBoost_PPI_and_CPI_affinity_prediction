"""
Applies the RAW/vanilla (use_lora=False, never fine-tuned) T5ProtChem-native
encoder mean-pool + XGBoost predictor, trained on split_pure_random_uniqueonly
(swap-pair duplicates removed BEFORE splitting) with CPI:PPI ~1:1
oversampling (mutants included) and quadrupled ProtSMILES splice range
(12-32 residues) -- val overall pearson 0.797, test overall pearson 0.810 --
to every protein-compound pair in
/home/tomita/Hoshino_polymer/predicted_pKa_KanM.csv.
"""
...
OUTPUT_CSV = "/home/tomita/Hoshino_polymer/predicted_pKa_KanM_T5ProtChem_raw_uniqueonly_quadsplice.csv"
...
def extract_feature(protein_seq, smiles):
    molA_text = text_encoder.encode_aa(protein_seq)
    molB_text = text_encoder.encode_smiles(smiles)
...
pred_pKd = reg.predict(X)

df["predicted_pKd_T5ProtChem_raw_uniqueonly_quadsplice"] = pred_pKd
df.to_csv(OUTPUT_CSV, index=False)
print(f"Saved predictions to {OUTPUT_CSV}")
print(df[["n", "m", "pKd", "predicted_pKd_T5ProtChem_raw_uniqueonly_quadsplice"]].to_string(index=False))
